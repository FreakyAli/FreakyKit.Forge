...
public sealed class ExpressionGeneratorTests : GeneratorTestBase
{
    ...
    [Fact]
    public void Phase2_Condition_OnMember_ExcludedFromExpression_FKF506()
    {
        // Regression test: Condition guards the imperative assignment with `if (Method(source))`,
        // but has no expression-tree equivalent. Previously this was silently dropped — the
        // expression contained an unconditional assignment that diverged from the imperative method.
        const string source = """
            using FreakyKit.Forge;
            namespace TestNs
            {
                ...
                public static partial class F
                {
                    ...
                    internal static bool IsPositive(A source) => source.Value > 0;
                }
            }
            """;
        ...
        Assert.Contains(result.Diagnostics, d => d.Id == "FKF506");
        var generated = AssertSingleGeneratedFile(result);
        // Imperative method still guards the assignment
        Assert.Contains("if (IsPositive(source))", generated);
        // Expression omits Value entirely — since it was the only member, the property is suppressed
        Assert.DoesNotContain("MapExpression", generated);
    }

    [Fact]
    public void Phase2_IgnoreIfDefault_OnMember_ExcludedFromExpression_FKF506()
    {
        // Regression test: IgnoreIfDefault guards the imperative assignment but has no
        // expression-tree equivalent. Previously this was silently dropped from the expression.
        const string source = """
            using FreakyKit.Forge;
            namespace TestNs
            {
        ...
        Assert.Contains(result.Diagnostics, d => d.Id == "FKF506");
        var generated = AssertSingleGeneratedFile(result);
        Assert.Contains("EqualityComparer<string>.Default.Equals(source.Name, default)", generated);
        Assert.DoesNotContain("MapExpression", generated);
    }

    [Fact]
    public void Phase2_ConditionOnNestedForgeMember_ExcludedFromExpression_FKF506()
    {
        // Regression test: a nested-forge member with Condition must not have its expression
        // unconditionally inlined by the post-extraction ResolveExpressionInlining pass.
        const string source = """
            using FreakyKit.Forge;
            namespace TestNs
            {
                ...
                public class Person { public Address Home { get; set; } = new(); public int Age { get; set; } }
                public class PersonDto
                {
                    [ForgeMap("Home", Condition = "HasAddress")]
                ...
                public static partial class F
                {
                    ...
                    internal static bool HasAddress(Person source) => source.Home != null;
                }
            }
            """;
        ...
        var exprStart = generated.IndexOf("ToDtoExpression");
        Assert.True(exprStart >= 0);
        var exprBody = generated.Substring(exprStart);
        Assert.Contains("Age = source.Age", exprBody);
        Assert.DoesNotContain("Home =", exprBody);
    }
